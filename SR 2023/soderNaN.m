...
%
% Edited: 2019-03 Added the ability to ignore nan inputs
% Input:
...
indNancol = indNan(:,1) | indNan(:,2) | indNan(:,3);
x(indNancol,:) = [];
y(indNancol,:) = [];

mark_posnan = find(indNancol == 1);
for i = 1:length(mark_posnan)
    warning('Markers in position %i are NaNs.',mark_posnan(i));
end
